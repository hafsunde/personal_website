...
- [ ] **Set up the Pages build.** GitHub Action running `quarto render` + deploy, so publishing never needs Quarto installed locally.
  The publication list is generated by an R chunk (`R/publications.R`, via the `yaml` package), so the workflow needs
  `r-lib/actions/setup-r` and that one package alongside `quarto-actions/setup` — not Quarto on its own.
